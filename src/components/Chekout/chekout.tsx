...
const[error,setError]=useState<string>('')
    ...
    const submitHandler=(event:React.FormEvent<HTMLFormElement>)=>{
        ...
        if (!orderdata.name.trim() || !orderdata.email.trim() || !orderdata.street.trim() || !orderdata.postal.trim() || !orderdata.city.trim()) {
    ...
    return(
       ...
       <p className={classes['chekout__total']}>total Amount : ${Number(totalAmount).toFixed(2)}</p>
        ...
        <Input inline={false} label='PostalCode' type='text' id='postalCode' name='postal' minLength={5}/>
        ...
        <div className={classes['chekout__actions']} >
            ...
            <Buttons  className={classes['chekout__actions-submitter']}  label='submitOrder'  />
        </div>
        </Form>
